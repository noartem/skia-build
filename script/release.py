...
def main():
  ...
  try:
    resp = urllib.request.urlopen(urllib.request.Request('https://api.github.com/repos/noartem/SkiaBuild/releases/tags/' + version, headers=headers)).read()
  except urllib.error.URLError as e:
    data = '{"tag_name":"' + version + '","name":"' + version + '"}'
    resp = urllib.request.urlopen(urllib.request.Request('https://api.github.com/repos/noartem/SkiaBuild/releases', data=data.encode('utf-8'), headers=headers)).read()
